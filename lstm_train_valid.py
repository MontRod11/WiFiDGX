...
coordenadas_train = pd.read_csv("/home/laura/TrayectoriasLSTM/datasets/datasets_degradacion_testFijo/train/conjuntoTrain_numTrayectorias195_Nexus_difGPSfreqcon0_numTrayectoriasTest31_numTrayectoriasValid48.csv",header=None) 
txt ="conjuntoTrain_numTrayectorias195_Nexus_difGPSfreqcon0_numTrayectoriasTest31_numTrayectoriasValid48.csv"
...
coordenadas_valid= pd.read_csv("/home/laura/TrayectoriasLSTM/datasets/datasets_degradacion_testFijo/valid/conjuntoValid_numTrayectorias195_numTrayectoriasValid48_Nexus_difGPSfreqcon0_numTrayectoriasTest31.csv",header=None) 
txt_valid = 'conjuntoValid_numTrayectorias195_numTrayectoriasValid48_Nexus_difGPSfreqcon0_numTrayectoriasTest31.csv'
...
lr = 0.005637669307211288 # 0.004595 0.005637669307211288
momentum =  0.9699
num_epochs = 400
...
if sche == True:
    plt.savefig('models/train_valid_graph/model_numTrayectTrain'+str(numTrayect)+'seqtrain'+str(sequence_length_train)+'_numTrayectValid'+str(numTrayects_valid)+'_lr'+str(lr)+'_TeacherForcing'+str(teacher_forcing_ratio)+'_numLayers'+str(num_lay)+'_hiddNeurons'+str(hidd_d)+'_bs'+str(batch_size)+'_solape'+str(factor_solape)+'_optim'+str(optimizador)+'schedulerLRstepsize'+str(stepsize)+'gamma'+str(gamma)+'TestFijo.png',format='png', dpi=600)
else:
    plt.savefig('models/train_valid_graph/model_numTrayectTrain'+str(numTrayect)+'seqtrain'+str(sequence_length_train)+'_numTrayectValid'+str(numTrayects_valid)+'_lr'+str(lr)+'_TeacherForcing'+str(teacher_forcing_ratio)+'_numLayers'+str(num_lay)+'_hiddNeurons'+str(hidd_d)+'_bs'+str(batch_size)+'_solape'+str(factor_solape)+'_optim'+str(optimizador)+'NoSchedulerTestFijo.png',format='png', dpi=600)
...
def check_accuracy_imgs_seq(loader, net, dataset, name, loss_function, h_state, seq_len, col, test_batch_size, device, minmax, propiedades, nameprop, listado_titulo): #, num_classes, cols):
    ...
    with torch.no_grad():
        ...
        for step, (data, target) in enumerate(loader):
            ...
            plt.savefig(namedir+'/Pruebamap_tray'+str(step)+'_predGuadaAfuerasTrain.png',format='png', dpi=600)
...
if num_lay == 3:
    ...
    # new dataset:  
    predictions, targets = check_accuracy_imgs_seq(train_loader, model, 'train','conjunto_test_fijo/numtrayectstrain'+str(numTrayect)+'_numtrayectsvalid'+str(numTrayects_valid)+'_seqlen'+str(sequence_length_train)+'_TeacherForcing'+str(teacher_forcing_ratio), loss_func, train_state,sequence_length_train,in_size,batch_size, device, minmax, propiedades, nameprop, lista_dias_train)
    predictions, targets = check_accuracy_imgs_seq(valid_loader, model, 'valid','conjunto_test_fijo/numtrayectstrain'+str(numTrayect)+'_numtrayectsvalid'+str(numTrayects_valid)+'_seqlen'+str(sequence_length_train)+'_TeacherForcing'+str(teacher_forcing_ratio), loss_func, valid_state,sequence_length_valid,in_size,tbatch_size, device, minmax, propiedades, nameprop, lista_dias_valid)
elif num_lay == 2:
    # new dataset:  
    if sche == True:
        if optimizador == 1:
        ...
        if optimizador == 1:
            ...
            nameprop='lr'+str(lr)+'_momentum'+str(momentum)+'batchsize'+str(batch_size)+'hidd'+str(hidd_d)+'numlay'+str(num_lay)+'solape'+str(factor_solape)+'lonsec'+str(sequence_length_train)+'_optim'+str(optimizador)+'NoScheduler'
    predictions, targets = check_accuracy_imgs_seq2lay(train_loader, model, 'train','conjunto_test_fijo/numtrayectstrain'+str(numTrayect)+'_numtrayectsvalid'+str(numTrayects_valid)+'_seqlen'+str(sequence_length_train)+'_TeacherForcing'+str(teacher_forcing_ratio), loss_func, train_state,sequence_length_train,in_size,batch_size, device, minmax, propiedades, nameprop, lista_dias_train)
    predictions, targets = check_accuracy_imgs_seq2lay(valid_loader, model, 'valid','conjunto_test_fijo/numtrayectstrain'+str(numTrayect)+'_numtrayectsvalid'+str(numTrayects_valid)+'_seqlen'+str(sequence_length_valid)+'_TeacherForcing'+str(teacher_forcing_ratio), loss_func, valid_state,sequence_length_valid,in_size,tbatch_size, device, minmax, propiedades, nameprop, lista_dias_valid)
else:
